...
Also note: Tank turns much better at low throttle (0-100) and most ideally at 0 throttle.
*/
...
/*bool CRaycastTankExample::OnEvent(const SEvent& event)
{
    ...
    switch(event.EventType)
    {
        ...
        case EET_KEY_INPUT_EVENT:
        {
            ...
            if(event.KeyInput.Key == KEY_KEY_Q && event.KeyInput.PressedDown == true)
            {
                leftTrackSpeed += 100.0;
                return true;
            }

            else
            if(event.KeyInput.Key == KEY_KEY_E && event.KeyInput.PressedDown == true)
            {
                rightTrackSpeed += 100.0;
                return true;
            }
...
bool CRaycastTankExample::OnEvent(const SEvent& event)
{
    ...
    switch(event.EventType)
    {
        ...
        case EET_KEY_INPUT_EVENT:
        {
            ...
            if(event.KeyInput.Key == KEY_KEY_A && event.KeyInput.PressedDown == true)
            {
                leftTrackBraking = brakingIncrement;
                return true;
            }
            ...
            if(event.KeyInput.Key == KEY_KEY_D && event.KeyInput.PressedDown == true)
            {
                rightTrackBraking = brakingIncrement;
                return true;
            }

            else
            if(event.KeyInput.Key == KEY_KEY_D && event.KeyInput.PressedDown == false)
            {
                rightTrackBraking = 0.0;
                return true;
            }*/
            ...
            if(event.KeyInput.Key == KEY_SPACE && event.KeyInput.PressedDown == false)
            {
                ...
                //createMuzzleSmoke(node->getJointNode("Muzzle"));
...
{
    for(s32 i=0; i<KEY_KEY_CODES_COUNT; i++) KeyIsDown[i] = false;
    ...
    device->setWindowCaption(L"irrBullet Raycast Tank");
...
void CRaycastTankExample::updateTank()
{
    ...
    else
    {
        ...
        if(IsKeyDown(KEY_KEY_F))
        {
            ...
            //FightersTW.getVehicleByID(vehicleID).getRigidBody().getLinearVelocity();
        }
    }
    ...
    /*vehicle->applyEngineForce(leftTrackSpeed,0);
    vehicle->applyEngineForce(leftTrackSpeed,1);
    vehicle->applyEngineForce(leftTrackSpeed,2);
    vehicle->applyEngineForce(leftTrackSpeed,3);
    vehicle->applyEngineForce(rightTrackSpeed,4);
    vehicle->applyEngineForce(rightTrackSpeed,5);
    vehicle->applyEngineForce(rightTrackSpeed,6);
    vehicle->applyEngineForce(rightTrackSpeed,7);*/
}
